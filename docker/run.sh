...
set_model_params() {
    local model_key=$(echo "$MODEL_NAME" | tr '[:lower:]-' '[:upper:]_')
    
    # Export the model parameters
    export VLLM_MODEL=$(eval echo \$${model_key}_MODEL)
    export VLLM_GPU_MEMORY_UTILIZATION=$(eval echo \$${model_key}_GPU_MEMORY_UTILIZATION)
    export VLLM_MAX_MODEL_LEN=$(eval echo \$${model_key}_MAX_MODEL_LEN)
    export VLLM_EXTRA_ARGS=$(eval echo \$${model_key}_EXTRA_ARGS)
